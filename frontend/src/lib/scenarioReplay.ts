...
const DEFAULT_OPTIONS: Omit<Required<GenerateReplayOptions>, "baseTsMs"> = {
...
export function generateScenarioReplay(options: GenerateReplayOptions = {}): GeneratedReplay {
  const config: Omit<Required<GenerateReplayOptions>, "baseTsMs"> & { baseTsMs?: number } = {
    ...DEFAULT_OPTIONS,
    ...options,
  };
  ...
  const now = config.baseTsMs ?? Date.now();
