...
async function generateScenarioReplayBundle(): Promise<void> {
  const generated = generateScenarioReplay({
    replayId: "scenario_replay_01",
    scenarioId: "faculty_phish",
    totalSteps: 200,
    seed: 1003,
    includeBlue: true,
    baseTsMs: 1_710_100_300_000,
  });

  const timeline = indexReplayMessages(generated.messages);
  const root = path.resolve(process.cwd(), "episodes/replays", generated.replayId);
  ...
  const manifest = {
    ...
    checkpoint_id: "ckpt_blue_scenario_0001",
  ...
  console.log(`Scenario replay written to ${root}`);
}

void generateScenarioReplayBundle();
